...
public class XMockCpuConversions {
    private static final String TAG = "XLua.MockCpuConversions";

    public static XMockCpu fromBundle(Bundle b) {
        if(b == null)
            return null;
        ...
        XMockCpu cpu = new XMockCpu();
        cpu.fromBundle(b);
        return cpu;
    }

    public static Collection<XMockCpu> fromBundleArray(Bundle bundle) {
        ...
        Collection<XMockCpu> items = new ArrayList<>();
        ...
        for (int i = 0; i < names.length; i++)
            items.add(new XMockCpu(names[i], models[i], manufacturers[i], contents[i], selected[i]));

        return items;
    }

    public static Bundle toBundleArray(Collection<XMockCpu> maps) {
        Bundle b = new Bundle();
        if(maps == null)
            return b;
